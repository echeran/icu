...
void dumpBidiMirroredGlyph(UProperty uproperty, FILE* f) {
    ...
    // Since the return value for u_charMirror(cp), when the code point doesn't
    // have a mirroring glyph, is the code point itself, we should expect the
    // max value to be larger than 16 bits wide.
    UCPTrieValueWidth width = UCPTRIE_VALUE_BITS_32;
    ...
    for(UChar32 c = UCHAR_MIN_VALUE; c <= UCHAR_MAX_VALUE; c++) {
        UChar32 mirroredGlyph = u_charMirror(c);
        ...
        // instead of the ICU API behavior of returning the code point value.
        if (c != mirroredGlyph) {
            umutablecptrie_set(builder.getAlias(), c, mirroredGlyph, status);
        }
    }
...
int exportUprops(int argc, char* argv[]) {
    ...
    for (const char* propName : propNames) {
        ...
        if (propEnum < UCHAR_BINARY_LIMIT) {
            ...
            dumpBidiMirroredGlyph(propEnum, f);
